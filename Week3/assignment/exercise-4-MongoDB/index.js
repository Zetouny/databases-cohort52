...
async function findEpisodesExercises(client) {
  ...
  const findEpisodeTitle = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .findOne({ episode: "S02E02" });

  console.log(
    `The title of episode 2 in season 2 is ${findEpisodeTitle.title}`
  );

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  const findSeasonAndEpisode = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .findOne({ title: "BLACK RIVER" });

  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${findSeasonAndEpisode.episode}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]
  const findEpisodesWithCliff = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: "CLIFF" });
  ...
  const findEpisodesWithCliffAndLighthouse = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: { $all: ["CLIFF", "LIGHTHOUSE"] } });

  const EpisodesWithCliffAndLighthouseArray = [];
  for await (const episode of findEpisodesWithCliffAndLighthouse) {
    EpisodesWithCliffAndLighthouseArray.push(episode.title);
  }

  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${EpisodesWithCliffAndLighthouseArray.join(
      ", "
    )}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const updateEpisode13 = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .updateOne(
  ...
  console.log(
    `Ran a command to update episode 13 in season 30 and it updated ${updateEpisode13.modifiedCount} episodes`
  );
...
async function deleteEpisodeExercise(client) {
  ...
  console.log(
    `Searching for episode 14 in season 31 after deleting returns ${verifyDeletion}, which verify the deletion`
  );
}
